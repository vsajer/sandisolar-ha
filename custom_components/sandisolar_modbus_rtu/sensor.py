...
class SimpleSensor(BaseSandiSensor):
    ...
    async def async_update(self):
        ...
        self._attr_native_value = int(val) if self._decimals == 0 else round(val, self._decimals)
...
class WorkTimeSensor(BaseSandiSensor):
    ...
    async def async_update(self):
        ...
        # registr je v jednotkách 0.5 min
        hours = (val * 0.5) / 60
...
class BatteryStatusTextSensor(BaseSandiSensor):
    ...
    async def async_update(self):
        ...
        self._attr_native_value = BATTERY_STATUS_MAP.get(val, f"Raw BMS status: {val}")
...
class GridStatusTextSensor(BaseSandiSensor):
    ...
    async def async_update(self):
        ...
        self._attr_native_value = GRID_STATUS_MAP.get(val, "Unknown inverter status")
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    entities += [
        ...
        SimpleSensor(hub, "bms_fcc", "Battery FCC",
                     "Ah", None, "mdi:battery-heart", 1),
        SimpleSensor(hub, "bms_rm", "Battery RM",
                     "Ah", None, "mdi:battery-clock", 1),
        SimpleSensor(hub, "bms_cycle_count", "Battery Cycle Count",
                     None, None, "mdi:counter", 0),
        SimpleSensor(hub, "bms_soh", "Battery SOH",
                     PERCENTAGE, None, "mdi:battery-check", 0),
        ...
        # Load / House
        SimpleSensor(hub, "home_load_power", "Home Load Power",
                     UnitOfPower.WATT, SensorDeviceClass.POWER, "mdi:home-lightning-bolt", 0),
        SimpleSensor(hub, "self_power", "Self Consumption Power",
                     UnitOfPower.WATT, SensorDeviceClass.POWER, "mdi:solar-power-variant", 0),
        SimpleSensor(hub, "system_power", "System Power",
                     UnitOfPower.WATT, SensorDeviceClass.POWER, "mdi:flash", 0),

        EnergySensor(hub, "load_energy_today", "Load Energy Today", "mdi:home-lightning-bolt"),
        EnergySensor(hub, "load_energy_total", "Load Energy Total", "mdi:home-lightning-bolt"),
